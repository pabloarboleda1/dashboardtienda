"""
====================================================================
 REGISTRAR VENTA - GESTOR DE INVENTARIO TIENDA VIRTUAL UCN
====================================================================
...
from common import inyectar_estilos, encabezado, render_kpi, formato_pesos, ACENTO_INGRESOS, ACENTO_UNIDADES
from db import obtener_sesion, solicitar_conexion_si_falta
from models import Producto, Venta, TasaRetribucion, CANALES_VENTA

st.set_page_config(page_title="Registrar Venta | Tienda Virtual UCN", page_icon="📝", layout="wide")
tema = inyectar_estilos()
encabezado("Registrar Venta", "Elige la prenda y la talla — el sistema calcula el resto solo.")
...
if not productos_activos:
    st.warning("No hay productos con stock disponible. Ve a 'Catálogo de Productos' para agregar o reabastecer.")
    sesion.close()
    st.stop()
...
opciones_talla = {f"{p.talla} (stock: {p.stock_actual})": p for p in tallas_disponibles}
talla_elegida_label = col_talla.selectbox("Talla", list(opciones_talla.keys()))
producto = opciones_talla[talla_elegida_label]

st.caption(f"Precio de venta sugerido: {formato_pesos(producto.precio_venta_unitario)} · Stock disponible: {producto.stock_actual}")
